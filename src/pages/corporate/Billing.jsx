...
const PLANS = [
  { id:'starter', name:'Starter', price:5000, seats:'Up to 50', features:['2 programmes','Basic analytics','Email support','Enrolment reports'], color:'#6b7280' },
  { id:'growth', name:'Growth', price:15000, seats:'Up to 200', features:['5 programmes','Advanced analytics','VITA AI chatbot','Custom campaigns','Priority support'], color:'#3b82f6', popular:true },
  { id:'enterprise', name:'Enterprise', price:35000, seats:'Unlimited', features:['All programmes','Full analytics suite','Complete AI features','SSO integration','HRMS connectors','Dedicated account manager','Custom programmes'], color:'#7c3aed' },
]
...
function UpgradeModal({ open, onClose }) {
  ...
  const subtotal = (plan?.price || 0) + seatCost
        ...
        <div className="grid grid-cols-3 gap-3">
          {PLANS.map(p => (
            <button key={p.id} onClick={() => setSelected(p.id)}
              ...
              <div className="text-xl font-display font-bold" style={{color:p.color}}>₹{(p.price/1000).toFixed(0)}k<span className="text-xs font-normal text-slate-500">/yr</span></div>
              <div className="text-xs text-slate-500 mt-1">{p.seats}</div>
            </button>
          ))}
        </div>
        <div>
          ...
          <div className="card p-6 bg-gradient-to-br from-slate-900 to-slate-800 text-white border-0">
            <div className="flex items-start justify-between mb-6">
              <div>
                ...
                <h2 className="font-display text-3xl text-white">Enterprise Plan</h2>
                <p className="text-slate-400 text-sm mt-1">Annual contract · Auto-renewal enabled</p>
              </div>
              <div className="text-right">
                <div className="font-display text-3xl text-white">₹1,59,300</div>

      ...
      {tab === 'overview' && (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              { icon: Key, label: 'Two-Factor Authentication', status: 'Partial', detail: '2 of 4 admins have MFA enabled', color: 'text-orange-500', bg: 'bg-orange-50', action: 'Enforce MFA', onClick: () => toast('MFA action triggered', 'info') },
              { icon: Lock, label: 'Password Policy', status: 'Active', detail: 'Update your admin password', color: 'text-emerald-600', bg: 'bg-emerald-50', action: 'Change Password', onClick: () => setShowPasswordModal(true) },
              ...
              <div key={s.label} className="card p-5 flex items-center gap-4">
                ...
                <button onClick={s.onClick || (() => toast(`${s.action} action triggered`, 'info'))} className="btn btn-sm btn-secondary text-xs">{s.action}</button>
              </div>
            ))}
          </div>

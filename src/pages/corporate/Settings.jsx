...
export default function SettingsPage() {
  ...
  const adminName = admin?.profile?.firstName ? `${admin.profile.firstName} ${admin.profile.lastName || ''}`.trim() : (admin?.name || '');
  const { toast } = useUIStore()
  const [tab, setTab] = useState('profile')
  const [inviteOpen, setInviteOpen] = useState(false)
  const [apiKeyVisible, setApiKeyVisible] = useState({})
  const [form, setForm] = useState({ name: adminName, jobTitle: admin?.corporateProfile?.designation || admin?.jobTitle || '', mobile: admin?.profile?.phoneNumber || admin?.mobile || '' })
      ...
      {tab === 'profile' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <div className="card p-6 flex flex-col items-center text-center">
            ...
            <h3 className="font-semibold text-slate-900">{adminName || 'Admin'}</h3>
            <p className="text-sm text-slate-500">{admin?.corporateProfile?.designation || admin?.jobTitle || 'Admin'}</p>
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="card p-6">
              ...
              <button className="btn-primary btn mt-4" onClick={() => { toast('Profile update requires backend integration', 'info') }}>
                Save Changes
              </button>
            </div>
